...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for x in valid_words:
        cur_len = diff_function(user_word, x,limit)
        if cur_len < minDiff:
            ans = x
            minDiff = cur_len
    if minDiff > limit:
...
def report_progress(typed, prompt, id, send):
    ...
    for t,p in zip(typed, prompt):
        if t == p:
            ...
            break
    send(({'id': id, 'progress': leng / len(prompt)}))
    return leng / len(prompt)
    # END PROBLEM 8
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        timei = []
        prev = times_per_player[i][0]
        for j in range(1,len(times_per_player[i])):
            timei += [times_per_player[i][j]-prev]
            prev = times_per_player[i][j]
        times += [timei]
...
def fastest_words(game):
    ...
    #     ipwords = []
    #     for iw in words:
    #         base = time(game, ip, iw)
    #         flag = False
    #         for jp in players:
    #             if time(game,jp,iw) < base :
    #                 flag = True
    #             elif time(game,jp,iw) == base and jp < ip:
    #                 flag = True
    #         if not flag:
    #             ipwords += [word_at(game, iw)]
    #     fatest += [ipwords]
    # 更简便版本
    words = all_words(game)
    times = all_times(game)
    fatest = [[] for i in range(len(players))]
    for  i,w in enumerate(words):
        time_list = [times[j][i] for j in players]
        idx = min(range(len(players)), key=lambda x:time_list[x])
        fatest[idx] += [w]
    return fatest
