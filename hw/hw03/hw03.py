...
def composer(func=lambda x: x):
    ...
    def func_adder(g):
        "*** YOUR CODE HERE ***"
        cur_comp = lambda x:func(g(x))
        return  composer(cur_comp)    
...
def g_iter(n):
    ...
    if n<=3:
        return n
    lft1,lft2,lft3 = 1,2,3
    i = 4
    while i<=n:
        lft1,lft2,lft3 = lft2,lft3,3*lft1+2*lft2+lft3
        i+=1
    return lft3
...
def missing_digits(n):
    ...
    cur_miss = 0
    if n%10-n//10%10-1 >= 0:
        cur_miss = n%10-n//10%10-1
    return missing_digits(n//10)+cur_miss
def count_change(total):
    ...
    def helper(n, minLen):
        ...
        if n<(1<<minLen):
            return 0
        return helper(n-(1<<minLen),minLen)+helper(n,minLen+1)
